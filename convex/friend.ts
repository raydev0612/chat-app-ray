...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!conversation) {
      throw new ConvexError("Converzacion no encontrada");
    }
    ...
    if (!memberships || memberships.length !== 2) {
      throw new ConvexError("Esta conversación no tiene mas miembros");
    }
    ...
    if (!friendships) {
      throw new ConvexError("No se encontro un amigo");
    }
