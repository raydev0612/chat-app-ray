...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!identity) {
      throw new ConvexError("Sin autorización");
    }

    if (args.email === identity.email) {
      throw new ConvexError("No puede enviarse una solicitud a sí mismo");
    }
    ...
    if (!receiver) {
      throw new ConvexError("No se ha podido encontrar al usuario");
    }
    ...
    if (requestAlreadyReceived) {
      throw new ConvexError("Este usuario ya le ha enviado una solicitud");
    }
...
export const deny = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!request || request.receiver !== currentUser._id) {
      throw new ConvexError("Hubo un error, solicitud denegada");
    }
  ...
  handler: async (ctx, args) => {
    ...
    if (!request || request.receiver !== currentUser._id) {
      throw new ConvexError(
        "Se ha producido un error al aceptar esta solicitud"
      );
    }
